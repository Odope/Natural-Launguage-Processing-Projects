...
conversational_dataset = load_dataset("emozilla/soda_synthetic_dialogue")
f_step_num = 0
class Tokenizer:
    def __init__(self):
...
class Preprocessing:
    ...
    def preprocessing(self):
        ...
        dataset = load_dataset("emozilla/soda_synthetic_dialogue")
        dataset1 = dataset['train']
        length = len(dataset1)

        a = open('conversational_data.txt','w')
        z = 0
        for i in dataset1:
    ...
    def data_preperation(self):
        ...
        with open('tokenized_conversational.bin', mode='wb') as file:

            with open('conversational_data.txt', "r", encoding="utf-8") as input_file:
                bytes_read = 0
    ...
    def fetch_training_batch_old_version(self, tokens_per_batch, step_num):
        ...
        file = open('tokenized_conversational.bin','rb')
        total_bytes = os.path.getsize('tokenized_conversational.bin')
...
class Transformer(torch.nn.Module):
    ...
    def train_loop(self):
        ...
        save_interval = 4000
        saved_model_path = "Models/model.chatbot_752000"
        
        write_model_path = "Models/model.chatbot_"+str(i)
...
# Close the writer when done
    ...
    def sample(self,a):
        ...
        a = ' '.join(self.tokenizer.convert(pure_generation)).split(" ")
        z = 0
        for i in a:
            if (i == personb or i == persona) and z > 0:
...
class ChatApp(tk.Tk):
    ...
    def __init__(self):
        self.transformer = Transformer(False)
        self.conversation = ""
        super().__init__()
        global conversation
        ...
        self.messages_list.pack(side=tk.LEFT, fill=tk.BOTH, padx=5, pady=5)
        self.scrollbar.config(command=self.messages_list.yview)

        self.entry_field = tk.Entry(self)
        self.entry_field.pack(side=tk.BOTTOM, fill=tk.X, padx=5, pady=5)
        self.entry_field.bind("<Return>", self.send_message)

    def add_message(self, message):
        self.messages_list.insert(tk.END, f" {message}")
    ...
    def send_message(self, event=None):
        ...
        if message:
            ...
            bx = name + ": "
            self.conversation += bx
            sample1 = self.transformer.sample(self.conversation)
            self.conversation += sample1
            #Sample of self.conversation but just addmessage the sample, not the entire conversation and add the sample to self.conversation, and split the conversation
            self.add_message(bx+sample1)


# def destroy_messages(event):
    # content = messages_text.get("1.0", tk.END)
    ...
    # messages_text.destroy()


if __name__ == "__main__":

    # messages_text = tk.Text(height=30, width=52)
    # messages_text.pack(side=tk.TOP, fill=tk.BOTH, padx=5, pady=5)
    # messages_text.insert(
    #     tk.END,
    #     "create a character to talk to! ex. (personb: I am a farmer and i like farming tomatoes)"
    # )
    # messages_text.bind("<Return>", destroy_messages)
    
    app = ChatApp()
    print(greeting)
    app.add_message(greeting)
    app.mainloop()
